...
class FaceDetector(ROS2OpenCV3):
    ...
    def detect_face(self, input_image):
        ...
        new_size = (int(input_image.shape[1] / image_scale), int(input_image.shape[0] / image_scale))

        # convert color input image to grayscale
        #gray = cv2.cvtColor(input_image, cv2.COLOR_BGR2GRAY)

        # scale input image for faster processing
        small_img = cv2.resize(input_image, new_size, interpolation = cv2.INTER_LINEAR)

        #small_img = cv2.equalizeHist(small_img)

        # First check one of the frontal templates
        if self.cascade_1:
            faces = self.cascade_1.detectMultiScale(small_img, **self.haar_params)
                                         
        # If that fails, check the profile template
        if len(faces) == 0 and self.cascade_3:
            faces = self.cascade_3.detectMultiScale(small_img, **self.haar_params)

        # If that also fails, check a the other frontal template
        if len(faces) == 0 and self.cascade_2:
            faces = self.cascade_2.detectMultiScale(small_img, **self.haar_params)

        # The faces variable holds a list of face boxes.
        # If one or more faces are detected, return the first one.  
        if len(faces) > 0:
            face_box = faces[0]
            face_box[0] = face_box[0] * image_scale
            face_box[1] = face_box[1] * image_scale
            face_box[2] = face_box[2] * image_scale
            face_box[3] = face_box[3] * image_scale
